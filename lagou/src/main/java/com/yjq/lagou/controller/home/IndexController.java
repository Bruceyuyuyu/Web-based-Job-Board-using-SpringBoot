...
public class IndexController {
	...
	public String companyList(Model model,Page page,String search_value){
		...
		if(StringUtil.isEmpty(search_value)) {
			...
			//分页获取公司列表信息
			Page p = new Page();
			p.setTotalCount(companyService.getCompanyTotal("Certified"));
			...
			List<Company> findCompanyList = companyService.findCompanyList("Certified", page.getOffset(), 9);
			model.addAttribute("CompanyList", findCompanyList);
		}else {
			//如果search_value不为空
			Page p = new Page();
			p.setTotalCount(companyService.getCompanyTotalBySearchValue("Certified", search_value));
			p.setRows(9);//公司列表每页9个
			model.addAttribute("totalPage",p.getTotalPage());  //总页数
			model.addAttribute("currentPage",page.getPage()); //当前页
			List<Company> findCompanyList = companyService.findCompanyListBySearchValue("Certified", search_value, page.getOffset(), 9);
			model.addAttribute("CompanyList", findCompanyList);
			model.addAttribute("PositionList", positionService.findAll());
	...
	public String publishPosition(Model model,HttpServletRequest request,Long id){
		...
		if(findCompanyByUserId == null || !"Certified".equals(findCompanyByUserId.getState())) {
			model.addAttribute("errorMsg", "Your company has not passed the certification, please fill in the company information or wait for the certification! 3s for you to jump to my company page！");
		}
